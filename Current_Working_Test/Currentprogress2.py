...
class Opponent(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        elif self.state == self.MOVEONE:
            self.pause += 1
            if self.pause > self.delay:
                #reset pause and advance animation
                self.pause = 0
                self.frame += 1
                if self.frame >= len(self.opponent_moveoneImages):
                    self.frame = 0
                    self.state = self.STANDING
                    self.image = self.imageStand
                else:
                    self.image = self.opponent_moveoneImages[self.frame]

        elif self.state == self.MOVETWO:
            self.pause += 1
            if self.pause > self.delay:
                #reset pause and advance animation
                self.pause = 0
                self.frame += 1
                if self.frame >= len(self.opponent_movetwoImages):
                    self.frame = 0
                    self.state = self.STANDING
                    self.image = self.imageStand
                else:
                    self.image = self.opponent_movetwoImages[self.frame]

        elif self.state == self.MOVETHREE:
            self.pause += 1
            if self.pause > self.delay:
                #reset pause and advance animation
                self.pause = 0
                self.frame += 1
                if self.frame >= len(self.opponent_movethreeImages):
                    self.frame = 0
                    self.state = self.STANDING
                    self.image = self.imageStand
                else:
                    self.image = self.opponent_movethreeImages[self.frame]

        elif self.state == self.MOVEFOUR:
            self.pause += 1
            if self.pause > self.delay:
                #reset pause and advance animation
                self.pause = 0
                self.frame += 1
                if self.frame >= len(self.opponent_movefourImages):
                    self.frame = 0
                    self.state = self.STANDING
                    self.image = self.imageStand
                else:
                    self.image = self.opponent_movefourImages[self.frame]

        elif self.state == self.MOVEFIVE:
            self.pause += 1
            if self.pause > self.delay:
                #reset pause and advance animation
                self.pause = 0
                self.frame += 1
                if self.frame >= len(self.opponent_movefiveImages):
                    self.frame = 0
                    self.state = self.STANDING
                    self.image = self.imageStand
                else:
                    self.image = self.opponent_movefiveImages[self.frame]

        elif self.state == self.MOVESIX:
            self.pause += 1
            if self.pause > self.delay:
                #reset pause and advance animation
                self.pause = 0
                self.frame += 1
                if self.frame >= len(self.opponent_movesixImages):
                    self.frame = 0
                    self.state = self.STANDING
                    self.image = self.imageStand
                else:
                    self.image = self.opponent_movesixImages[self.frame]

        elif self.state == self.MOVESEVEN:
            self.pause += 1
            if self.pause > self.delay:
                #reset pause and advance animation
                self.pause = 0
                self.frame += 1
                if self.frame >= len(self.opponent_movesevenImages):
                    self.frame = 0
                    self.state = self.STANDING
                    self.image = self.imageStand
                else:
                    self.image = self.opponent_movesevenImages[self.frame]

        elif self.state == self.MOVEEIGHT:
            self.pause += 1
            if self.pause > self.delay:
                #reset pause and advance animation
                self.pause = 0
                self.frame += 1
                if self.frame >= len(self.opponent_moveeightImages):
                    self.frame = 0
                    self.state = self.STANDING
                    self.image = self.imageStand
                else:
                    self.image = self.opponent_moveeightImages[self.frame]

    def loadImages(self):
        self.imageStand = pygame.image.load("opponentImages/opponent_idle-e0001.png")
        self.imageStand = self.imageStand.convert()
        transColor = self.imageStand.get_at((1, 1))
        self.imageStand.set_colorkey(transColor)

        self.opponent_moveoneImages = []
        for i in range(10):  # needs to be 12
            imgName = "opponentImages/opponent_moveone-e000%d.png" % i
            ...
            self.opponent_moveoneImages.append(tmpImage)

        self.opponent_movetwoImages = []
        for i in range(7):
            imgName = "opponentImages/opponent_movetwo-e000%d.png" % i
            tmpImage = pygame.image.load(imgName)
            tmpImage = tmpImage.convert()
            transColor = tmpImage.get_at((1, 1))
            tmpImage.set_colorkey(transColor)
            self.opponent_movetwoImages.append(tmpImage)

        self.opponent_movethreeImages = []
        for i in range(5):
            imgName = "opponentImages/opponent_movethree-e000%d.png" % i
            tmpImage = pygame.image.load(imgName)
            tmpImage = tmpImage.convert()
            transColor = tmpImage.get_at((1, 1))
            tmpImage.set_colorkey(transColor)
            self.opponent_movethreeImages.append(tmpImage)

        self.opponent_movefourImages = []
        for i in range(10):   #needs to be 15 
        ...
        self.opponent_movefiveImages = []
        for i in range(6):
            imgName = "opponentImages/opponent_movefive-e000%d.png" % i
            tmpImage = pygame.image.load(imgName)
            tmpImage = tmpImage.convert()
            transColor = tmpImage.get_at((1, 1))
            tmpImage.set_colorkey(transColor)
            self.opponent_movefiveImages.append(tmpImage)

        self.opponent_movesixImages = []
        for i in range(4):
            imgName = "opponentImages/opponent_movesix-e000%d.png" % i
            tmpImage = pygame.image.load(imgName)
            tmpImage = tmpImage.convert()
            transColor = tmpImage.get_at((1, 1))
            tmpImage.set_colorkey(transColor)
            self.opponent_movesixImages.append(tmpImage)

        self.opponent_movesevenImages = []
        for i in range(4):
            imgName = "opponentImages/opponent_moveseven-e000%d.png" % i
            tmpImage = pygame.image.load(imgName)
            tmpImage = tmpImage.convert()
            transColor = tmpImage.get_at((1, 1))
            tmpImage.set_colorkey(transColor)
            self.opponent_movesevenImages.append(tmpImage)

        self.opponent_moveeightImages = []
        for i in range(2):
            imgName = "opponentImages/opponent_moveeight-e000%d.png" % i
            tmpImage = pygame.image.load(imgName)
            tmpImage = tmpImage.convert()
            transColor = tmpImage.get_at((1, 1))
            tmpImage.set_colorkey(transColor)
            self.opponent_moveeightImages.append(tmpImage)
...
playerone = Playerone()
opponent = Opponent()
allSprites = pygame.sprite.Group(playerone, opponent)
    ...
    for event in pygame.event.get():  # User did something
        if event.type == pygame.QUIT:  # If user clicked close
            ...
            if event.key == pygame.K_g:
                opponent.state = opponent.MOVEONE
            if event.key == pygame.K_h:
                opponent.state = opponent.MOVETWO
            if event.key == pygame.K_j:
                opponent.state = opponent.MOVETWO
            if event.key == pygame.K_k:
                opponent.state = opponent.MOVEFOUR
            if event.key == pygame.K_v:
                opponent.state = opponent.MOVEFIVE
            if event.key == pygame.K_b:
                opponent.state = opponent.MOVESIX
            if event.key == pygame.K_n:
                opponent.state = opponent.MOVESEVEN
            if event.key == pygame.K_m:
                opponent.state = opponent.MOVEEIGHT
